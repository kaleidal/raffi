...
export const mergeRemoteStateIntoLocal = (remote: RemoteState) => {
    ...
    for (const remoteItem of remote.library) {
        ...
        if (!localItem) library.set(key, { ...remoteItem });
        else if (syncState.dirty.library[key] || toTimestamp(localItem.last_watched) >= toTimestamp(remoteItem.last_watched)) {
            library.set(key, { ...remoteItem, ...localItem, poster: localItem.poster ?? remoteItem.poster, completed_at: localItem.completed_at ?? remoteItem.completed_at ?? null });
        } else {
            library.set(key, { ...localItem, ...remoteItem, poster: remoteItem.poster ?? localItem.poster, completed_at: remoteItem.completed_at ?? localItem.completed_at ?? null });
        }
    }
