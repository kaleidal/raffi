<script lang="ts">
    ...
    function isTimeBuffered(elem: HTMLVideoElement, target: number, tolerance = 0.5): boolean {
    ...
    const onSeek = (event: Event) => {
        ...
        const desiredGlobal = Math.max(0, Math.min(duration, duration - remaining));
    ...
    onMount(async () => {
        const testTorrentioSrc = "https://torrentio.strem.fun/resolve/realdebrid/LMDSM5K2GLBR4BG6MT6JBPYHR7C5HZP2RAUCCNL4ZIS7236LV2LA/17c6cc17891f3640cef082cde56fd426b1dcd016/null/2/Just.Add.Magic.S01E03.Just.Add.Dogs.2160p.AMZN.WEB-DL.DDP5.1.x265-LAZY.mkv";
        ...
        const sessionMetadata = await fetch(sessionUrl).then((res) => res.json());
    ...
    {#if controlsVisible}
        <div in:slide={{ duration: 200, axis: 'y' }}
            ...
            <div class="flex flex-row gap-[20px] items-center w-full">
                ...
                <span class="text-[22px] font-poppins font-[500] text-[#D3D3D3] text-center">{formatTime(Math.max(0, duration - currentTime))}</span>
