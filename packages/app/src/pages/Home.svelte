<script lang="ts">
    ...
    const getTitleKey = (title: PopularTitleMeta | undefined) =>
        title ? `${title.type}:${title.imdb_id || title.id}` : "";

    function keepCurrentFeaturedIfPresent(items: PopularTitleMeta[]) {
        const currentKey = getTitleKey(showcasedTitle);
        if (!currentKey) return false;
        return items.some((title) => getTitleKey(title) === currentKey);
    }

    function setFeaturedFromPool(options: { preserveCurrent?: boolean } = {}) {
        ...
        if (
            ...
            const traktCandidates = traktLogoCandidates.length > 0
                ? traktLogoCandidates
                : traktHeroCandidates.length > 0
                    ? traktHeroCandidates
                    : heroPoolTitles;
            if (options.preserveCurrent && keepCurrentFeaturedIfPresent(traktCandidates)) {
                return;
            }
        ...
        if (primaryCandidates.length > 0) {
            if (options.preserveCurrent && keepCurrentFeaturedIfPresent(primaryCandidates)) {
                return;
            }
        ...
        if (fallbackCandidates.length > 0) {
            if (options.preserveCurrent && keepCurrentFeaturedIfPresent(fallbackCandidates)) {
                return;
            }
        ...
        if (options.preserveCurrent && keepCurrentFeaturedIfPresent(looseCandidates)) {
            return;
        }
    ...
    async function refreshHeroSourcePool(
    ...
    async function refreshPersonalHomeSections(
        installedAddons?: Addon[],
        options: { preserveHero?: boolean } = {},
    ) {
        const selectedSourceId = getStoredHomeHeroSource();
        void loadContinueWatching();
        void refreshAddonSections(installedAddons);
        const heroOptions = { preserveCurrent: options.preserveHero };
        void loadTraktRecommendations().then(() => refreshHeroSourcePool(installedAddons, heroOptions));
        if (selectedSourceId !== HOME_HERO_SOURCE_TRAKT_RECOMMENDATIONS) {
            void refreshHeroSourcePool(installedAddons, heroOptions);
        }
    }
    ...
    onMount(() => {
        ...
        const handleHomeRefresh = async (event: Event) => {
            const installedAddons = await getAddons().catch(() => [] as Addon[]);
            const preserveHero = event instanceof CustomEvent
                ? event.detail?.preserveHero !== false
                : true;
            void refreshPersonalHomeSections(installedAddons, { preserveHero });
        };
